...
from curve_fit import build_results_table, create_comparison_table
...
def main():
    ...
    cache_dir="Cache"
    cache_filename = os.path.join(cache_dir, f"ZTF_survey_{param_hash}.parquet")

    if os.path.exists(cache_filename):
        print("Cached ZTF-survey data found. Loading ztf.survey from file...")
        ztf_survey_df=pd.read_parquet(cache_filename)
        ztf_survey=skysurvey.Survey()
        ztf_survey.set_data(ztf_survey_df)
        ...
        ztf_survey_df=ztf_survey.data
        ztf_survey_df.to_parquet(cache_filename)
    ...
    comparison_table = create_comparison_table(fit_results_table=results_table)
